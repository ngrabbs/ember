"""RF screening model, not a fabrication acceptance model.

Usage: python rf_filter_vendor.py inventory.json output_dir '[settings JSON]'
Inductor: Coilcraft Document 158 (2026-04-29), 0402HP-10N.
Z = R2 + ((Rvar + jwL) || (R1 + 1/jwC)); Rvar=k*sqrt(f).
Capacitor/ground/pad parameters are user-specified sensitivity assumptions.
Ports are 50 ohm and active devices/bias branches are excluded.
"""
import sys,pathlib
input_file=sys.argv[1];output_dir=pathlib.Path(sys.argv[2]);output_dir.mkdir(parents=True,exist_ok=True)
import numpy as np,json,math
settings=json.loads(sys.argv[3]) if len(sys.argv)>3 else {}
def coilcraft_z(ff,L):
 w=2*np.pi*ff
 return .085+1/(1/(2.57e-5*np.sqrt(ff)+1j*w*L)+1/(4+1/(1j*w*.051e-12)))
...
f=np.array(settings.get('frequencies_MHz',list(np.arange(350,501,.25))))*1e6;result={}
assert np.all((f>=1e6)&(f<=10e9)), 'Outside manufacturer model frequency range'
for name,refs in groups.items():
 ...
 # Collapse each component terminal to the nearby routed endpoint; pad effects are parameterized separately.
 elements=[];pads={}
 for c in cs:
  ns=[]
  for p in c['pads']:
 ...
 for ff in f:
  ...
  for typ,v,a,b in elements:
   if typ=='C':
    if abs(v-9.1e-12)<1e-15:v=settings.get('shunt_pF',9.1)*1e-12
    elif abs(v-3.9e-12)<1e-15:v=settings.get('io_pF',3.9)*1e-12
    elif abs(v-1.6e-12)<1e-15:v=settings.get('coupling_pF',1.6)*1e-12
    impedance=1/(1j*w*v)+settings.get('cap_ESR_ohm',0)+1j*w*settings.get('cap_ESL_nH',0)*1e-9
   else:impedance=coilcraft_z(ff,v)
   if a<0 or b<0:impedance+=1j*w*settings.get('ground_L_nH',0)*1e-9
   stamp(a,b,1/impedance)
  for idx in pads.values():
   if idx>=0:Y[idx,idx]+=1j*w*settings.get('pad_C_pF',0)*1e-12
  for a,b in edges:
   theta=w*math.sqrt(settings.get("epsilon_eff",3.3))/299792458*math.dist(pts[a],pts[b])/1000
 ...
 vals=np.array(vals);pk=np.argmax(vals[:,0]);result[name]={'internal_trace_mm':sum(math.dist(pts[a],pts[b]) for a,b in edges),'nodes':len(pts),'edges':len(edges),'sampled_peak_MHz':float(f[pk]/1e6),'sampled_peak_S21_dB':float(vals[pk,0]),'435MHz':vals[int(np.argmin(abs(f-435e6)))].tolist(),'437MHz':vals[int(np.argmin(abs(f-437e6)))].tolist()}
 np.savetxt(output_dir/('comms-filter-'+name+'.csv'),np.c_[f/1e6,vals],delimiter=',',header='MHz,S21_dB,S11_dB',comments='')
json.dump({'settings':settings,'filters':result,'inductor_435MHz':{'R_ohm':float(coilcraft_z(435e6,10e-9).real),'X_ohm':float(coilcraft_z(435e6,10e-9).imag)}},open(output_dir/'results.json','w'),indent=2)
